...
description: Design Apple-style interfaces following Human Interface Guidelines. This is the DEFAULT style for all UI/frontend work. Creates HIG-compliant components with SF Pro typography, system colors, and proper accessibility. Automatically activated for any UI creation unless user specifies a different style.
---

# Apple HIG Designer

Apple Human Interface Guidelines に準拠したプロフェッショナルなインターフェースを設計するスキル。
**UI/フロントエンド作業のデフォルトスタイル** — 別のスタイルが明示的に指定されない限り、常にこのスキルを適用する。
...
- フロントエンドページの新規作成
- Webアプリケーションの画面設計
- HTML/CSS/React等のUI実装

**明示的トリガー:**
- 「Apple風に」「HIG準拠で」「iOS/macOSスタイルで」
- 「UIを作って」「画面を設計して」「フロントエンドを実装して」
- "Design an Apple-style..." / "Create a HIG-compliant..."

**適用しない場合（ユーザーが明示的に指定）:**
- 「Material Designで」「Bootstrapスタイルで」「独自デザインで」等

## UX心理学との連携

UIを設計・レビューする際は、`~/.claude/knowledge/ux-psychology/` の知識を併用する。
Apple HIG の「どう作るか」に、UX心理学の「なぜ人間に効くか」を組み合わせることで、根拠のあるデザイン判断が可能になる。
...
| 段階的なオンボーディング | 段階的開示 | `ux-psychology/information.md` §2 |
