...
| 4 | **Windows MCP** | Windows操作の自動化（ウィンドウ/プロセス/レジストリ） | 開発環境の自動構築・検証 |
...
### 4-3. 選択されたMCPの前提条件リサーチ（都度実行）

⚠️ **重要**: MCPの前提条件（ランタイム・バージョン・APIキー・インストールコマンド）は**頻繁に変更される**ため、このプロンプトにハードコードしない。
**選択されたMCPごとに、以下の手順で最新情報を都度リサーチすること。**

#### リサーチ手順（選択されたMCP1つにつき1回）:

1. **WebSearch** で「`[MCP名] mcp server install prerequisites`」を検索
2. 公式リポジトリ（GitHub README）または公式ドキュメントを **WebFetch** で取得
3. 以下の情報を抽出:
   - **ランタイム**: Node.js (npx) / Python (uvx) / Docker / その他 — 必要バージョンも確認
   - **APIキー**: 必要か否か。必要な場合は取得先URL
   - **インストールコマンド**: `npx -y <package>` / `uvx <package>` / `docker run ...` 等
   - **`claude mcp add` コマンド**: ターミナル版での追加コマンド

4. リサーチ結果をユーザーに以下の形式で報告:

```
📋 [MCP名] の前提条件（最新情報を確認済み）:
  ランタイム: [Node.js 18+ / Python 3.13+ / Docker 等]
  APIキー:   [不要 / 必要 → 取得先URL]
  コマンド:  [npx -y ... / uvx ... / docker run ...]
```

5. 4-1で確認済みの環境と照合し、**不足しているランタイムやバージョンがあれば自動インストールを実施**
   - 例: Python 3.13+が必要だがインストールされていない → インストール手順を案内または自動実行

**Claude Preview** は標準搭載のためリサーチ不要。

### 4-4. MCP追加手順の案内

前提条件が整ったら、以下の手順を**テキストで案内するだけ**にしてください。
（⚠️ `settings.json` への書き込みや `npx`/`uvx` コマンドの実行は一切行わないこと）

**ユーザーの環境に応じて、該当する方を案内してください:**

#### デスクトップアプリ（GUI）の場合:
```
📋 MCPサーバーの追加手順:

1. Claude Code の設定を開く（左下の歯車アイコン）
2. 「MCP Servers」セクションを選択
3. 以下のMCPを名前で検索して「追加」:
   （選択されたMCPのみ表示）
4. 追加後、各MCPのステータスが ✅ になることを確認
```

#### ターミナル版（CLI）の場合:
4-3のリサーチで取得した `claude mcp add` コマンドを、選択されたMCPごとに案内してください。

```
📋 MCPサーバーの追加手順:

Claude Code を一旦終了してから、以下のコマンドを実行してください:
（4-3で調査した各MCPの claude mcp add コマンドをここに記載）

追加後、Claude Code を再起動してください。
```

（⚠️ 選択されたMCPのみ表示すること。スキップしたものは含めない）

### 4-5. MCP導入時のトラブルシューティング
...
| 品質管理ファイル（8個） | ✅ |
| 知識ベース（11ファイル） | ✅ |
| MCP前提条件 | ✅ リサーチ済・インストール済 / ⏭️ |
| MCP追加手順 | ✅ 案内済 / ⏭️ |

💡 MCPサーバー本体は上記の手順（GUI or CLI）で追加してください。
```
