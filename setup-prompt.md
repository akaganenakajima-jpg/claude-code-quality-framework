...
ユーザーが選択したMCPを、**グローバル設定ファイル `~/.claude/settings.json`** に書き込んで追加してください。

まず現在の `~/.claude/settings.json` を Read し、既存の `mcpServers` セクションがあればそこに追記、なければ新規作成してください。

各MCPの設定JSONは以下の通りです（選択されたもののみ追記）:
...
**⚠️ APIキー/トークンが必要なMCP（Notion / GitHub / Figma）の場合**:
- 設定を書き込む前に AskUserQuestion で「トークンを入力してください（後で設定する場合は空欄）」と聞く
- 空欄の場合は `YOUR_*_TOKEN` のままプレースホルダーを残し、後で編集するよう案内する

**Claude Preview** は標準搭載のため設定不要。

### 設定後の接続確認

設定ファイル書き込み後、ユーザーに以下を案内してください:

```
⚠️ MCPサーバーの設定を ~/.claude/settings.json に書き込みました。
   反映するには Claude Code を再起動してください（Ctrl+C → 再度起動）。
   再起動後、各MCPのツールが利用可能になります。
```

再起動後（または同一セッション内で反映された場合）、結果を報告してください:
```
✅ Notion — 設定済み
✅ Context7 — 設定済み
✅ Claude Preview — 標準搭載
✅ Windows MCP — 設定済み
✅ Desktop Commander — 設定済み
✅ PDF — 設定済み
⏭️ GitHub — スキップ
⏭️ Figma — スキップ
```
...
5. セクション4で選択したMCPサーバーが接続済み（スキップした場合は省略）
