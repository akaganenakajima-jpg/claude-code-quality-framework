...
選択されたMCPにAPIキーが必要な場合、**取得手順だけを案内**してください（⚠️ settings.jsonへの書き込みやnpxコマンドの実行は行わないこと）:

| MCP | 必要なキー | 取得先 |
|-----|-----------|--------|
| Notion | Integration Token | `https://www.notion.so/my-integrations` → New integration |
| GitHub | Personal Access Token | `https://github.com/settings/tokens` → Generate new token → `repo` スコープ |
| Figma | API Key | `https://www.figma.com/developers/api#access-tokens` → Personal Access Token |

該当するMCPが選択されている場合のみ、上記を案内してください。
Claude Preview / Context7 / Windows MCP / Desktop Commander / PDF はAPIキー不要です。

### 4-4. MCP追加手順の案内

前提条件とAPIキーの準備が整ったら、以下の手順を**テキストで案内するだけ**にしてください。
（⚠️ `settings.json` への書き込みや `npx` コマンドの実行は一切行わないこと）
...
# Windows MCP
claude mcp add windows-mcp -s user -- npx -y @anthropic-ai/windows-mcp

# Desktop Commander
claude mcp add desktop-commander -s user -- npx -y @wonderwhy-er/desktop-commander

# PDF
claude mcp add pdf -s user -- npx -y @sylphx/pdf-reader-mcp

# GitHub（YOUR_TOKEN は取得済みのものに置換）
claude mcp add github -s user -e GITHUB_TOKEN=YOUR_TOKEN -- npx -y @modelcontextprotocol/server-github

# Figma（YOUR_KEY は取得済みのものに置換）
