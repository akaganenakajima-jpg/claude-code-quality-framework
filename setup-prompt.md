...
## 3. IPA知識ベース（情報処理技術者試験 全10資格）

IPA情報処理技術者試験 全10区分の満点合格者レベルの専門知識を導入します。
このリポジトリの `knowledge/ipa/` フォルダにある11ファイルを `~/.claude/knowledge/ipa/` にコピーしてください。

```
~/.claude/knowledge/ipa/
├── index.md   ← 分野横断マップ・実務活用索引
├── fe.md      ← 基本情報技術者（アルゴリズム・データ構造・離散数学）
├── ap.md      ← 応用情報技術者（待ち行列理論・記述対策・深化ポイント）
├── db.md      ← データベーススペシャリスト（正規化・SQL高度技法・MVCC）
├── nw.md      ← ネットワークスペシャリスト（OSPF/BGP・VPN・SD-WAN）
├── es.md      ← エンベデッドシステムスペシャリスト（RTOS・安全設計・IoT）
├── pm.md      ← プロジェクトマネージャ（PMBOK・EVM・アジャイル）
├── sm.md      ← ITサービスマネージャ（ITIL4・SLA・BCP/DR）
├── st.md      ← ITストラテジスト（DX戦略・NPV/IRR・ビジネスモデル）
├── sa.md      ← システムアーキテクト（マイクロサービス・DDD・性能設計）
└── au.md      ← システム監査技術者（COSO・J-SOX・CAAT・COBIT）
```
...
関連業務が発生した際に該当ファイルを参照することで、満点合格者レベルの専門知識に基づいた設計判断・レビュー・助言が可能になります。

---

## 4. 推奨ツール設定（任意）

以下のMCPサーバーを追加すると、品質管理フレームワークの効果がさらに高まります。
**おすすめのツールを追加しますか？**（Claude Code GUI の設定画面から追加できます）

### おすすめ（追加を推奨）
1. **Notion MCP** — ドキュメントをNotionに自動記録・検索・更新できます
   - 追加方法: Claude Code 設定 → MCP Servers → 「Notion」を検索して追加
   - 品質管理での活用: レポート・議事録・インシデント記録の自動保存

2. **Context7 MCP** — npm/pip等のライブラリドキュメントを自動参照できます
   - 追加方法: Claude Code 設定 → MCP Servers → 「Context7」を検索して追加
   - 品質管理での活用: 「既存の解決策を使う」原則の実践支援

3. **Claude Preview** — Webページの見た目をスクリーンショットで確認できます
   - Claude Code に標準搭載（追加設定不要）
   - 品質管理での活用: UI検証の3層検証Layer2

### 任意（必要に応じて追加）
4. **Figma MCP** — Figmaのデザインファイルを参照できます（Figma APIキーが必要）
5. **Google Drive MCP** — Google Driveのファイルを読み取れます
6. **Box MCP** — Boxのファイルを検索・読み取り・アップロードできます
