...
タスクを受けると自分で課題を分析し、8分野の専門知識から必要な情報を引き出し、品質規律に従って作業する。UIを作れば自動的にApple HIG準拠のデザインを生成し、心理学的根拠まで添える — そんな認知フレームワークを Claude Code に導入します。
...
### 例2: UI作成（Apple HIG 自動適用）

「ダッシュボード画面を作って」と言うと:

1. **Apple HIG スキルが自動発動** → SF Pro Typography・System Colors・8pt Grid で実装
2. **UX心理学を併用** → 視覚的階層（`information.md` §3）、美的ユーザビリティ効果（`impression.md` §1）
3. **ダークモード対応** → `prefers-color-scheme` による自動切替
4. **心理学的根拠をコメントで明記** → なぜその配置・配色が効くかを説明

別のスタイルが必要な場合は「Material Designで」等と指定すればOK。
...
**UI作成のデフォルトスタイル** — 別のスタイルが指定されない限り、自動的に Apple Human Interface Guidelines に準拠したデザインを生成します。
UX心理学知識と連携し、「なぜそのデザインが効くか」の心理学的根拠も提供します。

| ファイル | 内容 |
|---------|------|
| `SKILL.md` | スキル定義・デザインシステム仕様・UX心理学連携テーブル |
| `REFERENCE.md` | ページレイアウト・ナビゲーション・フォーム等の実装パターン集 |
| `design-tokens.css` | CSS変数（SF Pro Typography・System Colors・8pt Grid・ダークモード） |
| `components.jsx` | Reactコンポーネントテンプレート（ボタン・カード・入力等） |
...
| HIG の設計判断 | 心理学的根拠 | 参照先 |
|---|---|---|
| 選択肢を最小限に | 決断疲れの回避 | `ux-psychology/choice.md` §7 |
| プログレスバー表示 | 目標勾配効果 | `ux-psychology/motivation.md` §1 |
| 破壊的操作に赤ボタン+確認 | 損失回避 | `ux-psychology/choice.md` §4 |
...
│       ├── REFERENCE.md            ← 実装パターン集
│       ├── LICENSE                 ← MIT（原作: axiaoge2/apple-hig-designer）
│       └── resources/
│           ├── design-tokens.css   ← CSS変数・カラー・タイポグラフィ
│           ├── components.jsx      ← Reactコンポーネントテンプレート
│           └── ui-patterns.md      ← UIパターン詳細
├── hooks/
...
### UIデフォルトスタイルの変更
Apple HIG 以外をデフォルトにしたい場合は、`CLAUDE.md` の「UI デザインのデフォルト」セクションを編集してください。
...
- フレームワーク本体: 組織内利用を想定
- Apple HIG デザインスキル: MIT（原作: [axiaoge2/apple-hig-designer](https://github.com/axiaoge2/apple-hig-designer)）
